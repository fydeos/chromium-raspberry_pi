...
ShellClientEventRouter::ShellClientEventRouter(Profile* profile) 
  : fydeos_client_observer_(this),
    ...
    weak_factory_(this){
  fydeos_client_observer_.Add(GetShellClient());
}
...
void ShellClientEventRouter::OnCommandNotificationReceived(int32_t handler, 
  ...
  if (!command_extension_ids_.size())
    return;
  std::unique_ptr<base::ListValue> args(new base::ListValue());
  args->AppendInteger(handler);
  args->AppendInteger(state);
  args->AppendString(msg);
  std::unique_ptr<Event> event(
...
void ShellClientEventRouter::OnCustomNotificationReceived(int32_t data,
  ...
  if (!custom_extension_ids_.size())
    return;
  std::unique_ptr<base::ListValue> args(new base::ListValue());
  args->AppendInteger(data);
  args->AppendInteger(exdata);
  args->AppendString(extra);
  std::unique_ptr<Event> event(
...
ExtensionFunction::ResponseAction ShellClientExecSyncFunction::Run() {
  std::unique_ptr<shell::ExecSync::Params> params(shell::ExecSync::Params::Create(*args_));
...
void ShellClientExecSyncFunction::StateCallback(base::Optional<ShellState> state) {
   Respond(OneArgument(CreateStateValueFromState(state)));
}

ExtensionFunction::ResponseAction ShellClientExecAsyncFunction::Run() {
  std::unique_ptr<shell::ExecAsync::Params> params(shell::ExecAsync::Params::Create(*args_));
  EXTENSION_FUNCTION_VALIDATE(params.get());
  GetShellClient()->AsyncExec(
      params->command,
      base::Bind(&ShellClientExecAsyncFunction::StateCallback, this)
    );
  return RespondLater();
}

void ShellClientExecAsyncFunction::StateCallback(base::Optional<ShellState> state) {
   Respond(OneArgument(CreateStateValueFromState(state)));
}

ExtensionFunction::ResponseAction ShellClientGetTaskStateFunction::Run() {
  std::unique_ptr<shell::GetTaskState::Params> params(shell::GetTaskState::Params::Create(*args_));
  EXTENSION_FUNCTION_VALIDATE(params.get());
  GetShellClient()->GetTaskState(
      params->key,
      base::Bind(&ShellClientGetTaskStateFunction::StateCallback, this)
    );
  return RespondLater();
}

void ShellClientGetTaskStateFunction::StateCallback(base::Optional<ShellState> state) {
   Respond(OneArgument(CreateStateValueFromState(state)));
}

ExtensionFunction::ResponseAction ShellClientForceCloseTaskFunction::Run() {
  std::unique_ptr<shell::ForceCloseTask::Params> params(shell::ForceCloseTask::Params::Create(*args_));
  EXTENSION_FUNCTION_VALIDATE(params.get());
  GetShellClient()->ForceCloseTask(
      params->key,
      base::Bind(&ShellClientForceCloseTaskFunction::StateCallback, this)
    );
  return RespondLater();
}

void ShellClientForceCloseTaskFunction::StateCallback(base::Optional<ShellState> state) {
   Respond(OneArgument(CreateStateValueFromState(state)));
}

ExtensionFunction::ResponseAction ShellClientGetTaskOutputFunction::Run() {
  std::unique_ptr<shell::GetTaskOutput::Params> params(shell::GetTaskOutput::Params::Create(*args_));
  EXTENSION_FUNCTION_VALIDATE(params.get());
  GetShellClient()->GetTaskOutput(
      params->key,
      params->lines,
      base::Bind(&ShellClientGetTaskOutputFunction::StateCallback, this)
    );
  return RespondLater();
}

void ShellClientGetTaskOutputFunction::StateCallback(base::Optional<ShellState> state) {
   Respond(OneArgument(CreateStateValueFromState(state)));
}

ExtensionFunction::ResponseAction ShellClientGetDaemonStateFunction::Run() {
  GetShellClient()->GetDaemonState(
      base::Bind(&ShellClientGetDaemonStateFunction::StateCallback, this)
    );
  return RespondLater();
}

void ShellClientGetDaemonStateFunction::StateCallback(base::Optional<ShellState> state) {
   Respond(OneArgument(CreateStateValueFromState(state)));
}
} //exit extensions
