...
bool AccelerometerFileReader::InitializeFydeOSAccelerometer(
  ...
  for (size_t i = 0; i < kNumberOfAxes; ++i) {
    ...
    if (pattern >= 0 && pattern < base::size(KFydeOSDataPattern))
    configuration_.index[ACCELEROMETER_SOURCE_SCREEN][i] =
...
//---***FYDEOS BEGIN***---
      ...
      VLOG(1) << "update: x:" << update_->get(source).x << " y:" << update_->get(source).y
        << " z:" << update_->get(source).z << " length:" << update_->GetVector(source).Length();
//---***FYDEOS END***---
    }
  }
